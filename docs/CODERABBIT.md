...
- `back/**` (.NET 10): que las integraciones externas en `Integrations/Nasa` e `Integrations/Secop`
  **degraden sin romper** (si NASA FIRMS o SECOP fallan -> `null` o lista vacia + log, nunca
  propagar la excepcion), que **todo `HttpClient` tenga timeout** y que **no haya credenciales en
  el codigo**.
