...
| B3 | **Falta crear el proyecto en Neon** (issue #4 — decisión ya tomada, ver D15). Hoy solo hay Docker local, así que cada uno tiene datos distintos | Infra | Datos de demo, despliegue |
...
| **PostgreSQL** | Base de datos | Gratis | Neon — decisión D15, falta que alguien cree el proyecto en `neon.tech` | ⬜ |
...
| D15 | **PostgreSQL en la nube: Neon** (issue #4), no Docker local ni Supabase/Railway | Neon tiene el free tier más simple para esto (sin tarjeta, proyecto en 5 minutos) y las 4 máquinas comparten los mismos datos de demo — Docker local ya nos tenía con "en mi máquina sí funciona". **Falta el paso manual:** alguien con acceso crea el proyecto en `neon.tech` y comparte la cadena de conexión por el grupo privado — eso no se puede hacer desde una sesión de agente. Mientras tanto, Docker local sigue funcionando igual (`docker compose up`) |
