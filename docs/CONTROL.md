...
| B4 | ~~Sin PostgreSQL en la nube~~ → **resuelto**: Azure Database for PostgreSQL Flexible Server en `conectariesgoai-rg` (región `brazilsouth`) | Infra | — |
...
| H3 | **Nadie tiene asignado conectar el backend con los microservicios** | 🟠 |
...
- **Tres microservicios** compilando con 0 errores y 0 advertencias:
  - `ms-transparencia` — probado con SECOP caído, devuelve el respaldo marcado
  - `ms-social` — acierta los 8 casos de prueba del clasificador
  - `ms-satelital` — responde correctamente sin credenciales
