...
| D3 | **Integraciones como clientes HTTP internos** en `back/src/ConectaRiesgoAI.Api/Integrations/` | Detalle en `docs/ARQUITECTURA.md`. ✅ **Ya ejecutada y sin huérfanos:** `Integrations/Nasa`, `Integrations/Secop` e `Integrations/Storage` existen y `GET /api/reportes/{codigo}` los consume. Los antiguos `servicios/ms-satelital`, `ms-transparencia` y `ms-social` nunca tuvieron código propio y se retiraron del repositorio — `Integrations/` es el único camino |
