...
| UI y documentación | `any` o `@ts-ignore`; control sin etiqueta asociada; interacción solo alcanzable con ratón. En C#, tipos o miembros con lógica sin XML doc en español **en cualquier visibilidad**: el compilador solo exige lo públicamente visible, así que una clase `internal` compila verde sin una línea (`/// <inheritdoc />` es suficiente) |
