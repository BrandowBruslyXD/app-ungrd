...
export default function Alerts() {
  ...
  const sourceConfig: Record<Alert['source'], { label: string; icon: typeof Satellite; color: string }> = {
    citizen: { label: t('alerts.sourceCitizen'), icon: AlertTriangle, color: 'text-ungrd-600 bg-ungrd-50' },
    satellite: { label: t('alerts.sourceSatellite'), icon: Satellite, color: 'text-emerald-600 bg-emerald-50' },
    social_media: { label: t('alerts.sourceSocial'), icon: MessageSquare, color: 'text-gold-700 bg-gold-50' },
    official: { label: t('alerts.sourceOfficial'), icon: Building, color: 'text-red-600 bg-red-50' },
  };
    ...
    <div className="mx-auto max-w-3xl px-4 py-6 lg:py-12 animate-fade-in">
      <div className="flex items-center gap-3 mb-4 sm:mb-6">
        ...
        <div className="min-w-0">
          <h1 className="text-xl font-bold text-slate-800 sm:text-2xl">{t('alerts.title')}</h1>
          <p className="text-sm text-slate-500">
            {t('alerts.inYourZone', { count: activeAlerts.length })}
          </p>
        </div>
      </div>
