...
const VERDICTS: readonly {
  ...
  { value: 'habitable', color: 'border-emerald-300 bg-emerald-50 text-emerald-800', iconColor: 'bg-emerald-200 text-emerald-700' },
  { value: 'uso_restringido', color: 'border-amber-300 bg-amber-50 text-amber-800', iconColor: 'bg-amber-200 text-amber-700' },
  { value: 'no_habitable', color: 'border-red-300 bg-red-50 text-red-800', iconColor: 'bg-red-200 text-red-700' },
];

const DAMAGE_OPTIONS: readonly { value: HousingDamageAggregate; color: string }[] = [
  { value: 'sin_dano', color: 'border-emerald-200 bg-emerald-50 text-emerald-700' },
  { value: 'averiada', color: 'border-amber-200 bg-amber-50 text-amber-700' },
  { value: 'destruida', color: 'border-red-200 bg-red-50 text-red-700' },
];

const VERDICT_BADGE: Record<Habitability, string> = {
  habitable: 'bg-emerald-50 text-emerald-700 ring-emerald-200',
  uso_restringido: 'bg-amber-50 text-amber-700 ring-amber-200',
  no_habitable: 'bg-red-50 text-red-700 ring-red-200',
};
...
export default function HabitabilityWizard() {
  ...
  if (submitted) {
    return (
      <div className="mx-auto max-w-lg px-4 py-16 text-center animate-scale-in" aria-live="polite">
        ...
        <h1 className="text-2xl font-bold text-slate-800">{t('habitability.successTitle')}</h1>
        <p className="mt-3 text-base text-slate-500 leading-relaxed">
          {t('habitability.successText')}
          {form.habitability === 'no_habitable' ? ` ${t('habitability.successEvacuation')}` : ''}
        </p>
        <div className="mt-6 card p-5">
          <p className="text-sm text-slate-500">{t('habitability.assessmentCode')}</p>
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8 lg:py-12 animate-fade-in">
      <h1 className="text-xl font-bold text-slate-800 lg:text-2xl mb-1">{t('habitability.title')}</h1>
      <p className="text-sm text-slate-500 mb-6">{t('habitability.subtitle')}</p>
      ...
      {step === 1 && (
        <div className="space-y-4 animate-slide-up">
          <div>
            <label htmlFor="hab-incident" className="block text-sm font-medium text-slate-700 mb-1.5">
          ...
          <div>
            <label htmlFor="hab-address" className="block text-sm font-medium text-slate-700 mb-1.5">
      ...
      {step === 2 && (
        <div className="space-y-5 animate-slide-up">
          <div>
            <p className="block text-sm font-medium text-slate-700 mb-2">{t('habitability.verdict')}</p>
          ...
          <div>
            <p className="block text-sm font-medium text-slate-700 mb-2">{t('habitability.damageCategory')}</p>
          ...
          <div>
            <label htmlFor="hab-structural" className="flex items-center gap-3 cursor-pointer rounded-xl border border-slate-200 bg-slate-50 p-4 transition-colors hover:bg-slate-100">
              <input
                id="hab-structural"
                type="checkbox"
              ...
              />
              <div>
                <span className="text-sm font-medium text-slate-700">{t('habitability.structuralInspection')}</span>
                <p className="text-xs text-slate-500 mt-0.5">{t('habitability.structuralInspectionHint')}</p>
              </div>
            </label>
          </div>

          <div>
            <label htmlFor="hab-notes" className="block text-sm font-medium text-slate-700 mb-1.5">
      ...
      {step === 3 && (
        <div className="space-y-4 animate-slide-up">
          {form.habitability === 'no_habitable' && (
            <div className="rounded-2xl border-2 border-red-300 bg-red-50 p-5">
              <div className="flex items-start gap-3">
                <FileWarning className="h-6 w-6 text-red-600 shrink-0 mt-0.5" aria-hidden="true" />
                <div>
                  <h3 className="font-bold text-red-800">{t('habitability.notHabitableTitle')}</h3>
                  <p className="text-sm text-red-700 mt-1 leading-relaxed">{t('habitability.notHabitableText')}</p>
                </div>
              </div>
            </div>
          )}

          <label htmlFor="hab-evacuation" className="flex items-center gap-3 cursor-pointer rounded-xl border border-slate-200 bg-white p-4 transition-colors hover:bg-slate-50">
            <input
              id="hab-evacuation"
              type="checkbox"
            ...
            />
            <div>
              <span className="text-sm font-medium text-slate-700">{t('habitability.evacuationIssued')}</span>
              <p className="text-xs text-slate-500 mt-0.5">{t('habitability.evacuationIssuedHint')}</p>
            </div>
          </label>

          <label htmlFor="hab-shelter" className="flex items-center gap-3 cursor-pointer rounded-xl border border-slate-200 bg-white p-4 transition-colors hover:bg-slate-50">
            <input
              id="hab-shelter"
              type="checkbox"
            ...
            />
            <div>
              <span className="text-sm font-medium text-slate-700">{t('habitability.shelterActivated')}</span>
              <p className="text-xs text-slate-500 mt-0.5">{t('habitability.shelterActivatedHint')}</p>
            </div>
          </label>

          <div className="card p-5 mt-4">
            <h3 className="font-bold text-slate-800 mb-3">{t('habitability.summaryTitle')}</h3>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-slate-500">{t('habitability.summaryAddress')}</span>
                <span className="font-medium text-slate-700 text-right max-w-[60%]">{form.address}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">{t('habitability.summaryType')}</span>
                <span className="font-medium text-slate-700">
                  {form.housingType ? t(`habitability.housingTypes.${form.housingType}`) : t('habitability.emptyValue')}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-500">{t('habitability.summaryHabitability')}</span>
                {form.habitability && (
              ...
              <div className="flex justify-between">
                <span className="text-slate-500">{t('habitability.summaryDamage')}</span>
                <span className="font-medium text-slate-700">
                  {form.damageAggregate ? t(`census.damageAggregate.${form.damageAggregate}`) : t('habitability.emptyValue')}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">{t('habitability.summaryOccupants')}</span>
                <span className="font-medium text-slate-700">{form.occupantsPresent}</span>
              </div>
              {form.needsStructuralInspection && (
                <div className="flex justify-between">
                  <span className="text-slate-500">{t('habitability.summaryInspection')}</span>
                  <span className="font-medium text-amber-600">{t('habitability.inspectionRequired')}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="rounded-xl border border-blue-200 bg-blue-50 p-4">
            <p className="text-sm text-blue-800">{t('habitability.edanNote')}</p>
          </div>
        </div>
      )}
