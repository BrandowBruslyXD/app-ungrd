...
export default function StepConsentReview({ data, update }: Props) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-lg font-bold text-slate-900">{t('census.consent.title')}</h2>
        <p className="mt-1 text-sm text-slate-500">{t('census.consent.subtitle')}</p>
      </div>

      <div className="rounded-xl border-2 border-ungrd-200 bg-ungrd-50 p-4 sm:p-5">
        <div className="flex items-start gap-3">
          <Shield className="mt-0.5 h-5 w-5 shrink-0 text-ungrd-600" aria-hidden="true" />
          <div>
            <h3 className="text-sm font-bold text-ungrd-900">{t('census.consent.privacyTitle')}</h3>
      ...
      <div className="space-y-3 rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
        <h3 className="text-sm font-bold text-slate-800">{t('census.consent.summaryTitle')}</h3>

        <div className="flex items-start gap-3 border-b border-slate-100 pb-3">
          <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-ungrd-500" aria-hidden="true" />
        ...
        <div className="flex items-start gap-3 border-b border-slate-100 pb-3">
          <Home className="mt-0.5 h-4 w-4 shrink-0 text-amber-500" aria-hidden="true" />
          <div className="text-sm min-w-0">
            <p className="font-medium text-slate-800 truncate">{data.address}</p>
            <p className="text-xs text-slate-500">{damageText}</p>
        ...
        <div className="flex items-start gap-3 border-b border-slate-100 pb-3">
          <Users className="mt-0.5 h-4 w-4 shrink-0 text-blue-500" aria-hidden="true" />
          <div className="text-sm min-w-0">
            <p className="font-medium text-slate-800">
        ...
        <div className="flex items-start gap-3">
          <Package className="mt-0.5 h-4 w-4 shrink-0 text-gold-600" aria-hidden="true" />
          <div className="text-sm min-w-0">
            <p className="font-medium text-slate-800">{t('census.consent.needsTitle')}</p>
            {data.families.map((f, i) => (
              <p key={f.id} className="text-xs text-slate-500 truncate">
      ...
      <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
        <h3 className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-500">{t('census.consent.checksTitle')}</h3>
      ...
      <label htmlFor="census-consent" className="flex cursor-pointer items-start gap-3 rounded-xl border-2 border-slate-200 bg-white p-4 transition-colors hover:border-ungrd-300">
        <input
          id="census-consent"
          type="checkbox"
        ...
        />
        <div>
          <p className="text-sm font-medium text-slate-800">{t('census.consent.authorizeTitle')}</p>
          <p className="mt-0.5 text-xs text-slate-500">{t('census.consent.authorizeText')}</p>
        </div>
      </label>
    </div>
  );
