...
export default function StepHousingVisit({ data, update }: Props) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-lg font-bold text-tinta-900">{t('census.housing.title')}</h2>
        <p className="mt-1 text-sm text-tinta-500">{t('census.housing.subtitle')}</p>
      </div>

      <div>
        <label htmlFor="census-address" className="etiqueta">
          {t('census.housing.address')} <span className="text-alerta-500">{t('census.requiredMark')}</span>
        </label>
        <input
          id="census-address"
          type="text"
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <label htmlFor="census-housing-type" className="etiqueta">
        ...
        <div>
          <label htmlFor="census-ownership" className="etiqueta">
      ...
      <div>
        <p className="etiqueta">
          {t('census.housing.families')} <span className="text-alerta-500">{t('census.requiredMark')}</span>
        </p>
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex h-10 w-10 items-center justify-center rounded-xl border border-tinta-200 text-lg font-bold text-tinta-600 hover:bg-tinta-50"
          >
            -
          </button>
          <span className="flex h-10 w-14 items-center justify-center rounded-xl border border-tinta-200 bg-tinta-50 text-lg font-bold text-tinta-900">
      ...
      <div>
        <p className="mb-2 block text-sm font-medium text-tinta-700">{t('census.housing.services')}</p>
          ...
          {services.map((svc) => (
            <button
              ...
              className={`flex items-center gap-2 rounded-xl border-2 px-4 py-2.5 text-sm font-medium transition-colors ${
                data[svc.key]
                  ? 'border-alerta-400 bg-alerta-50 text-alerta-700'
